...
\end{figure}
図~\ref{fig:test}に鶏むね肉を対象とし，提案手法による反力推定と力センサの計測結果を示す．反力の最大値と反力の減少傾向が力センサの計測結果と類似していることがわかる．
\begin{figure}[htbp]
    \centering
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{compare_cycle_MAE.png}
...
$\mathbf{q}$に$\boldsymbol{f}$を用いた場合のMAEが最も小さく，$\boldsymbol{symbol{f}}$に関しては，積分を重ねるごとにMAEが増加することがわかる．
よって，提案した同定計算は，グリッパの軌道に適しており，反力推定精度を向上させていることがわかる．
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{select5mode.png}
